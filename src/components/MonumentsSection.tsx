...
const MonumentsSection = ({ data = [], locale = "en" }: { data?: Monument[], locale?: string }) => {
  return (
      ...
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-12 grid-rows-2 gap-7 pt-14 transition duration-[800ms] ease">
          {data.map((monument, index) => {
            const typedMonumentTitle = monument.name as {
              [key: string]: string;
            };
            
            return (
            <Link
              key={`monument-${monument.id}-${index}`}
              href="/"
            ...
            >
              <div className="absolute top-0 start-0 w-full h-full z-[-1] overflow-hidden box-border">
                <Image
                  ...
                  src={monument.image ? monument.image : placeholderImg}
                  alt="Monument Image"
              ...
              <div className="flex items-end w-full h-full">
                <h4 className="text-white text-xl">{typedMonumentTitle[locale] ? typedMonumentTitle[locale] : typedMonumentTitle["hu"]}</h4>
              </div>
            </Link>
          )})}
          {/* <Link
