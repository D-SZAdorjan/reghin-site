...
const UpdateMonumentCategoryPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_ROUTE}monument-categories?id=${slug}`); // API call
        const jsonData = await response.json();
        const data = SuperJSON.deserialize<ArticleCategory>(jsonData.data);
        setFormData({
          name: JSON.stringify(data.name || {de:"", en: "", fr: "", hu: "", ro: ""}),
          priority: parseInt(data.priority!!.toString()),
        });

        const typedNameData = data.name as { [key: string]: string };
        setName({
          de: typedNameData.de || "", 
          en: typedNameData.en || "", 
          fr: typedNameData.fr || "", 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.patch(
        `/api/monument-categories?id=${slug}`, // Your route
        apiData,
        {
          headers: {
...
export default UpdateMonumentCategoryPage;