...
export async function GET(req: NextRequest){
    ...
    if(id){
        data = await prisma.articleCategory.findUnique({
            where: {
                id: BigInt(id),
            },
        });
        ...
        try {
            data = await prisma.articleCategory.findMany();
        } catch (error) {
            console.error(error);
    ...
    return NextResponse.json(
        {
            sucess: true,
            message: 'List Data ArticleCategories',
            data: serializedData,
        },
        {
            status: 200,
...
export async function POST(request: NextRequest){
    ...
    const rowData: Prisma.ArticleCategoryCreateInput = {
        name: name,
        priority: priority
    };
    
    // Create table row
    const response = await prisma.articleCategory.create({ data: rowData });
...
export async function PATCH(request: NextRequest) {
    ...
    let rowData: Prisma.ArticleCategoryUpdateInput;
    ...
    // Create table row
    const response = await prisma.articleCategory.update({ 
        where: {
          id: BigInt(id!!),
        },
        data: rowData
